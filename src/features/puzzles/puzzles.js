...
const PUZZLE_DEFS = [
  ...
  },
  {
    ...
    hint: "Nothing says two embedding rows have to point at different logits.",
    vocab: ["?", "a", "b"],
    maxLen: 4,
    epsilon: 0.05,
    tests: [
  ...
  },
  {
    ...
    hint: "W_E and W_P are added together — give row 0 of W_P enough weight to outrank whatever W_E contributes.",
    vocab: ["▶", "a", "b"],
    inputVocab: ["a", "b"],
    maxLen: 4,
    epsilon: 0.05,
  ...
  },
  {
    ...
    hint: "A 7-wide embedding solves it but costs 49 weights. The answer is one bit — narrow the stream down to a single dimension and let a Linear expand it back to the vocabulary.",
    vocab: ["a", "e", "i", "b", "c", "V", "C"],
  ...
  },
  {
    ...
    formula: "y_i = x_0",
    hint: "A mask column of zeros with -inf everywhere else points the whole sequence at one position. The head can write straight back onto the token dims if it shouts louder than the embedding.",
    vocab: ["a", "b", "c"],
    maxLen: 4,
    epsilon: 0.05,
    tests: [
  ...
  },
  {
    ...
    formula: "y_i = x_{3-i}",
    hint: "Same overwrite as Broadcast, but the mask is the anti-diagonal instead of a single column.",
    vocab: ["a", "b", "c"],
    maxLen: 4,
  ...
  },
  {
    ...
    hint: "W_E and W_P only ever add, and no sum of a token term and a position term can flip sign four times like this. Put the token sign in one dim and the position sign in another, then let an MLP's relu multiply them.",
    vocab: ["a", "b"],
    maxLen: 4,
    epsilon: 0.05,
    tests: [
  ...
  },
  {
    ...
    hint: "Bring x_0 alongside x_i in its own slice of the stream. Comparing two one-hots is not linear, but relu(a_k + b_k - 1) fires only where both carry a 1.",
    vocab: ["a", "b", "c", "y", "n"],
    inputVocab: ["a", "b", "c"],
    maxLen: 4,
    epsilon: 0.05,
  ...
  },
  {
    ...
    hint: "With W_Q and W_K at zero every score is equal, so a causal mask turns the head into a running average. Averaging a 0/1 indicator gives the fraction of a's, and a Linear bias can put the decision boundary just above one half.",
    vocab: ["a", "b", "y", "n"],
    inputVocab: ["a", "b"],
    maxLen: 6,
    epsilon: 0.05,
  ...
  },
  {
    ...
    hint: "Two heads in series. The first tags every position with its predecessor; the second queries with the current token against those tags, and copies the token it lands on.",
    vocab: ["a", "b", "c"],
    maxLen: 6,
    epsilon: 0.05,
    tests: [
